...
def modificar_funcion():
    ...
    try:
        with open(ARCHIVO_FUNCIONES, "r", encoding="utf-8") as arch_orig, open(ARCHIVO_TEMP, "w", encoding="utf-8") as arch_temp:
            for linea in arch_orig:
                ...
                try:
                    ...
                    if id_actual == id_modificar:
                        encontrada = True
                        print(f"Función encontrada: Obra {partes[1]}, Fecha {partes[2]}")
                        nuevaFecha = input("Ingrese la fecha nueva (enter para dejar igual): ").strip()
                        nuevaObra = input("Ingrese nuevo ID de obra (enter para dejar igual): ").strip()
                        ...
                        if nuevaObra != "":
                            try:
                                ...
                                print("ID de obra inválido, se mantendrá el ID original.")
                                obra_final = partes[1]
            ...
            for linea in arch_orig:
                ...
                try:
                    ...
                    if id_actual == id_borrar:
                        if confirmacion(f"¿Seguro que quiere borrar la función {partes[0]}? (s/n): "):
                            encontrado = True
